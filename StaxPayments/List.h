...
template <typename T> class ListImpl { // Implementation class
 ...
 ListImpl(const T& actualValue, ListImpl* prevImpl = nullptr, ListImpl* nextImpl = nullptr):value(actualValue),prev(prevImpl),next(nextImpl)
  ...
  // No copy constructor, assignment operator, move constructor and operator
...
public:
  ...
  void push_front(const T& value); // Add to the beginning
...
template <typename T> class ListIter {
  friend List<T>;
  static void remove(List<T>& list, const T& t);// Remove one specific element from the list with the value
  static void remove_all(List<T>& list, const T& t);// Remove all elements with a given value from the list
  // No constructors
...
template <typename T> void List<T>::push_back(const T& value) // Add to the end
  {
...
template <typename T>  void List<T>::pop_back(){ // remove last elem    
    if( backImpl_) {
      if( backImpl_->prev){
...
template <typename T> void List<T>::pop_front(){ // remove first elem
...
template <typename T>  void List<T>::print(const char separator,std::ostream& out)const{
...
template <typename T>
void List<T>::print(const char* separator,std::ostream &out)const{
...
template <typename T>
void List<T>::clear() { // Clears out all entries.
